...
void readbutton(void *parameter) {
  while (1) {
    ...
    if (digitalRead(4)) {           //read the button press and make the data 1 2 3 to turn on corresponding led
      data = 1;
    }
    if (digitalRead(5)) {
      data = 2;
    ...
    if (data == 1 || data == 2 || data == 3) {  //if it is the correct data send it
...
void setup() {
  ...
  xTaskCreatePinnedToCore(readbutton, "read the data", 2048, NULL, 2, NULL, app_cpu);
